-- v1.7 업데이트 공지 발송 — 초대 링크 공유 기능 (문의 반영)
...
-- ⚠️ App Store 심사 승인·출시 후에 실행 (공지가 먼저 나가면 업데이트가 없는데 안내되는 상태)
